...
import NoLinkedPatientEmptyState from '../components/NoLinkedPatientEmptyState';
import { 
  Activity, 
  Pill, 
  AlertOctagon, 
...
export default function CaregiverDashboard({ currentView, onViewChange, user, onLogout }) {
  ...
  const { data: linkedData, loading: linkedLoading, execute: loadLinked } = useApi(linkApi.getLinkedUsers);
  ...
  // Determine whether the caregiver has any approved linked patients.
  // Only load patient-specific data after confirming a linked patient exists.
  const linkedUsers = linkedData?.linked_users;
  const hasLinkedPatients = Array.isArray(linkedUsers) && linkedUsers.length > 0;

  useEffect(() => {
    if (!hasLinkedPatients) return;  // Do not load patient data when no link exists
    loadSummary();
    loadAdherence();
    loadEmergencies();
    loadBehavior();
    ...
    loadEvents();
    loadMedicines();
  }, [hasLinkedPatients, loadSummary, loadAdherence, loadEmergencies, loadBehavior, loadWellness, loadEvents, loadMedicines]);

  // ─── LOADING STATE ───────────────────────────────────────────────────────────
  if (linkedLoading || linkedData === null) {
    return (
      <div className="w-full flex items-center justify-center py-20">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 rounded-full border-2 border-blue-500/30 border-t-blue-400 animate-spin" />
          <p className="text-slate-400 text-sm">Loading caregiver dashboard…</p>
        </div>
      </div>
    );
  }

  // ─── NO LINKED PATIENT GUARD ─────────────────────────────────────────────────
  // When a caregiver has zero approved linked patients, render the empty state.
  // No patient-specific UI, no mock values, no fabricated data is shown.
      ...
      <ErrorBoundary>
        {activeEmergency && (
          <CaregiverEmergencyBanner
            ...
            onAcknowledge={() => setActiveEmergency(prev => prev ? { ...prev, status: 'acknowledged' } : null)}
            onDismiss={() => { stopEmergencySound(); setActiveEmergency(null); }}
          />
        )}
        <NoLinkedPatientEmptyState onViewChange={onViewChange} />
      </ErrorBoundary>
    );
  }

  // ─── ACTIVE PATIENT ───────────────────────────────────────────────────────────
  // Only reached when at least one linked patient is confirmed.
  const activeSubjectId = localStorage.getItem('orma_subject_id');
  const activePatient = linkedUsers.find(p => p.id === activeSubjectId) || linkedUsers[0];

  // ─── SAFE DATA DISPLAYS (zeros / empty — no hardcoded demo values) ────────────
  ...
  const displayBehavior = behavior?.confirmation_stats ? behavior : {
    confirmation_stats: { voice: 0, manual: 0, suspicious: 0, unverified: 0 },
    insights: []
  };
  ...
  const displayWellness = wellness?.status ? wellness : {
    status: 'No Data',
    emotions: { calmness: 0, anxiety: 0, sadness: 0 },
    confusion_events_recent: 0,
    repeated_questions: 0
  };

  const displayMedicines = Array.isArray(medicineList) && medicineList.length > 0
    ? medicineList
    : [];
          ...
          <div className="flex items-center gap-3.5 relative z-10">
            ...
            <div>
              ...
              <p className="text-slate-300 text-xs md:text-sm mt-0.5 font-medium">
                Monitoring: <span className="font-bold text-white">{activePatient?.name || '—'}</span> · Real-time adherence, alerts, and care activity
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 orma-card flex flex-col">
            ...
            <div className="flex-1 relative z-10">
              {displayMedicines.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 gap-2 border border-dashed border-white/10 rounded-2xl bg-slate-950/20">
                  <Pill className="w-7 h-7 text-slate-600" />
                  <p className="text-sm text-slate-500 font-medium">No medicines scheduled today</p>
                </div>
              ) : (
                <MedicationTimeline medicines={displayMedicines} mode="caregiver" />
              )}
            </div>
          </div>
          ...
          <div className="orma-card">
            ...
            <div className="space-y-3 relative z-10">
              ...
              {displayBehavior.insights.length === 0 && (
                <p className="text-xs text-slate-500 italic py-4 text-center">No behavioral observations recorded yet.</p>
              )}
            </div>
          </div>
